 ...
 */
public class AppUninstallTest extends SparkClientTestCase implements IThreadSafeTest {
    ...
    public void testExecutionSingle() throws Exception {
        resetAsyncLatch();
        mAppManager.uninstallApp("testHID_1").continueWith(new Continuation<Void, Void>() {
            @Override
    ...
    public void testExecutionSeries() throws Exception {
        ...
        mAppManager.uninstallApps(hidTaskSubset).continueWith(new Continuation<Void, Void>() {
            @Override
            public Void then(Task<Void> task) throws Exception {
                assertNull(task.getError());
                countDownAsyncLatch();
    ...
    public void testExecutionBulk() throws Exception {
        ...
        mAppManager.uninstallApps(getTestHIDs()).continueWith(new Continuation<Void, Void>() {
            @Override
            public Void then(Task<Void> task) throws Exception {
                assertNull(task.getError());
                countDownAsyncLatch();
