...
public class AppStopTest extends SparkClientTestCase implements IThreadSafeTest {
    ...
    public void testExecutionSingle() throws Exception {
        resetAsyncLatch();
        mAppManager.stopApp(SparkTerminalTestData.testPackageNames.get(0)).continueWith(new Continuation<Void, Void>() {
            @Override
    ...
    public void testExecutionSeries() throws Exception {
        ...
        mAppManager.stopApps(packageNames).continueWith(new Continuation<Void, Void>() {
            @Override
            public Void then(Task<Void> task) throws Exception {
                assertNull(task.getError());
                countDownAsyncLatch();
    ...
    public void testExecutionBulk() throws Exception {
        ...
        mAppManager.stopApps(SparkTerminalTestData.testPackageNames).continueWith(new Continuation<Void, Void>() {
            @Override
            public Void then(Task<Void> task) throws Exception {
                assertNull(task.getError());
                countDownAsyncLatch();
